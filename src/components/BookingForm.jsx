...
export default function BookingForm() {
  ...
  return (
    <div className="bg-stone-800 rounded-2xl p-6 md:p-8 shadow-xl">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <input name="date" type="date" value={form.date} onChange={handleChange} required className={inputClass} min={minDate} />
        ...
        <button
          ...
          className="sm:col-span-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-black font-bold py-3 rounded-full transition text-lg"
        >
          {status === 'loading' ? 'Booking...' : 'Confirm Booking'}
        </button>
        {status === 'success' && <p className="sm:col-span-2 text-green-400 text-center">✅ Booking confirmed! We'll see you soon.</p>}
        {status === 'error' && <p className="sm:col-span-2 text-red-400 text-center">❌ Something went wrong. Please try again.</p>}
      </form>
    </div>
  )
}
